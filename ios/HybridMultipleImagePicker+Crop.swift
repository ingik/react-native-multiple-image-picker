...
extension HybridMultipleImagePicker {
    ...
    func setCropConfig(_ cropConfig: PickerCropConfig) -> EditorConfiguration {
        ...
        if cropConfig.isSquare == true {
            ...
            // 1:1 비율만 표시
            config.cropSize.aspectRatios = [CropRatio(title: "1:1", width: 1, height: 1)]
            ...
            config.cropSize.isFixedRatio = true
            // 1:1.25 비율만 표시
            config.cropSize.aspectRatios = [CropRatio(title: "1:1.25", width: 1, height: 1.25)]
        } else {
            ...
            config.cropSize.aspectRatios = [
                CropRatio(title: "원본", width: 0, height: 0),
                CropRatio(title: "1:1", width: 1, height: 1),
                CropRatio(title: "3:4", width: 3, height: 4),
                CropRatio(title: "4:3", width: 4, height: 3),
                CropRatio(title: "9:16", width: 9, height: 16),
                CropRatio(title: "16:9", width: 16, height: 9)
            ]
        }
